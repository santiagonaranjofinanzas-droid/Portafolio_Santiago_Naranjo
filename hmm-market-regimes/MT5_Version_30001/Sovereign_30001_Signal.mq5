...
#property strict
#property tester_file "..\\Files\\HMM_Params_15M_30001.csv"
...
int OnInit() {
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "Sovereign 30001 Signal V8.1" );
    
    // V8: CSV parsing (keeps backward compat, static params used only as seed)
    int handle = FileOpen("HMM_Params_15M_30001.csv", FILE_READ  FILE_CSV  FILE_ANSI, ',');
    if(handle == INVALID_HANDLE) {
        Print("WARNING: HMM_Params_15M_30001.csv not found. Using built-in defaults.");
    ...
    if(!FileIsEnding(handle)) {
        ...
        if(StringLen(vals[0]) > 0) {
            ...
            PrintFormat("V8 Loaded CSV 30001: PBull=%.3f Lambda=%.3f Nu=%.2f WAccel=%.3f", ExtPBull, ExtJumpLambda, ExtHMMNu, ExtWAccel);
        }
    }
    FileClose(handle);
