...
#property strict
#property tester_file "..\\Files\\HMM_Params_15M_40001.csv"
#property tester_file "..\\Files\\Sovereign_Config_40001.csv"
...
input string InpRobustConfigCsv     = "Sovereign_Config_40001.csv";
...
double g_threshold     = 0.65;
double g_min_strength  = 0.3;
...
int OnInit() {
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "Sovereign 40001 Signal V8.1" );
    LoadRobustConfigCsv();
    
    // V8: CSV parsing (keeps backward compat, static params used only as seed)
    int handle = FileOpen("HMM_Params_15M_40001.csv", FILE_READ  FILE_CSV  FILE_ANSI, ',');
    if(handle == INVALID_HANDLE) {
        Print("WARNING: HMM_Params_15M_40001.csv not found. Using built-in defaults.");
...
void DrawDashboard() {
    ...
    string kalman_dir = (g_kalman_slope > 0) ? "UP" : (g_kalman_slope < 0) ? "DOWN" : "FLAT";
