...
input int      InpServerPort         = 5000;        // Python Server Port
...
input int      InpSocketTimeout      = 5000;        // Socket timeout in milliseconds
...
//+------------------------------------------------------------------+
// Rebalances a single symbol to its target weight (Certified v0.9.6)
//+------------------------------------------------------------------+
void RebalanceSymbol(string symbol, double weight)
{
   ...
   // Envío causal de órdenes al mercado
   if(diff > 0)
   {
      if(current_vol < 0)
      {
