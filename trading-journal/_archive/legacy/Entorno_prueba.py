...
import json
import uuid
import calendar # Nueva importación para la funcionalidad del calendario
...
st.markdown("""
    ...
    div.stButton > button {
        ...
        color: white;
        border: none;
        padding: 12px 28px; /* Incremento de padding para evitar solapamiento */
        border-radius: 8px;
        font-weight: 700;
...
def get_mt5_data(days_back=365):
    ...
    #  EXCEPCIÓN MANUAL SOLICITADA (05/01/2026)
    # Filtramos este día para evitar distorsión de métricas por outlier
    try:
        excl_date = date(2026, 1, 5)
        # Identificar filas
        mask_excl = trades['entrytime'].dt.date == excl_date
        # Calcular PnL a excluir para ajustar balance virtual
        pnl_excluded = trades.loc[mask_excl, 'netpnl'].sum()
        # Ajustamos el balance actual para simular que ese dinero nunca se perdió/ganó
        current_bal -= pnl_excluded
        # Eliminamos las filas del dataframe
        trades = trades[~mask_excl]
    except Exception as e:
        # En caso de error en el filtrado, continuamos sin filtrar para no romper la app
        pass
        ...
        # ==============================================================================
            ...
            if df_live.empty:
                ...
                with c_exp:
                    ...
                    st.dataframe(exposure.style.map(color_float, subset=['profit']).format({"net_vol": "{:.2f}", "profit": "${:.2f}"}), use_container_width=True)
