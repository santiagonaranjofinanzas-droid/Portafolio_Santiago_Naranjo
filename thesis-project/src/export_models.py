...
#parámetros FIJOS del training set, e incluye metadatos CLR.
#─────────────────────────────────────────────
print("\n[1/4] Exportando scaler_params.pkl y metadatos de caracteristicas...")

#Necesitamos los nombres de las columnas que XGBoost realmente espera
feature_names_xgb = ['ISRI'] + [f'CLR_State_{i}' for i in range(int(hmm_model.n_components))]

scaler_params = {
    "raw_asset_names": X_train.columns.tolist(),  # Para transformar los precios crudos entrantes
    "feature_names_xgb": feature_names_xgb,       # Estructura final que va al XGBoost
    "winsor_lower": X_train.quantile(WINSOR_LOWER_Q).to_dict(),
    "winsor_upper": X_train.quantile(WINSOR_UPPER_Q).to_dict(),
    "mu":             X_train.mean().to_dict(),
    "sigma":          X_train.std().to_dict(),
    "feature_version": FEATURE_VERSION,
    "model_version":   MODEL_VERSION,
    "n_regimes":       int(hmm_model.n_components),
    "clr_epsilon":     1e-6
}

path_scaler = os.path.join(OUTPUT_DIR, "scaler_params.pkl")
joblib.dump(scaler_params, path_scaler)
print(f"    [OK] Guardado: {path_scaler}")
print(f"    Raw assets ({len(scaler_params['raw_asset_names'])}): {scaler_params['raw_asset_names']}")
print(f"    XGBoost features ({len(scaler_params['feature_names_xgb'])}): {scaler_params['feature_names_xgb']}")
...
print(f"    [OK] Guardado: {path_hmm}")

#Verificacion rapida de matrices
if hasattr(hmm_model, "transmat_"):
    print(f"    Matriz de transicion:\n{np.round(hmm_model.transmat_, 3)}")
...
print(f"    [OK] Guardado: {path_prior}")
print(f"    Prior estacionario pi_0: {np.round(pi_0, 4)}")
...
manifest = {
    ...
    "raw_asset_names":  scaler_params["raw_asset_names"],
    "feature_names_xgb": scaler_params["feature_names_xgb"],
...
print("\n  Proximo paso: conectar estos artefactos en predict.py (FastAPI)")
print("=" * 50)
